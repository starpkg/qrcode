...
3. **`max_output_bytes` always active.** `maxOutput()` falls back to the 16 MiB default when the config value is unset or non-positive — a non-positive value must never silently disable the guard.
